...
export function calculateEstimatedPrice(partState: PartState): Dnum {
  const partModule = getPartModule(partState.type)
  // @ts-ignore
  const price = partModule.methods.calculateEstimatedPrice(partState)
  return dnum.from(price)
}
